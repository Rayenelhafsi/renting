...
(async()=>{
  ...
  const targets = [
    { ref:'REF-276', prefix:'biens/ref-276/images/' },
    { ref:'REF-283', prefix:'biens/ref-283/images/' },
  ];
  ...
  for (const t of targets) {
    ...
    if (mRows.length > keys.length) {
      ...
      if (toDelete.length) await db.query(`DELETE FROM media WHERE id IN (${toDelete.map(()=>'?').join(',')})`, toDelete);
    }
  ...
  await db.end();
  require('fs').writeFileSync('/var/www/dwiraimmobilier.com/public/fix-276-283-from-r2-folder-report.json', JSON.stringify(report,null,2));
