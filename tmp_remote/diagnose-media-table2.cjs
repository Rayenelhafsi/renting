...
(async () => {
  ...
  const [media] = await c.query(`SELECT bien_id,url,position,type FROM media WHERE bien_id IN (${placeholders}) ORDER BY bien_id,position,id`, ids);
  await c.end();

  const byId = new Map();
  for (const m of media) {
    if (!byId.has(m.bien_id)) byId.set(m.bien_id, []);
    byId.get(m.bien_id).push(m);
  }
  ...
  for (const b of biens) {
    ...
    for (const m of list.slice(0, 3)) {
      ...
      samples.push({ url: m.url, position: m.position, status: st.status, ok: st.ok, error: st.error || null });
    }
