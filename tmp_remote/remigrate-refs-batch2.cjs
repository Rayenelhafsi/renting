...
const TARGET = new Set(['ref-242','ref-243','ref-244','ref-246','ref-247','ref-248','ref-279','ref-280','ref-281']);
...
(async()=>{
  ...
  for(const ref of Array.from(TARGET)){
    ...
    try{
      ...
      for(const url of urls){
        ...
        await conn.query('INSERT INTO media (id,bien_id,type,url,motif_upload,position) VALUES (?,?,?,?,?,?)', [id,bienId,'image',url,'remigration_target_refs_batch2',p]);
        p += 1;
      }
      await conn.commit();
  ...
  fs.writeFileSync('/var/www/dwiraimmobilier.com/public/remigration-target-refs-batch2-report.json', JSON.stringify(report,null,2));
