...
const TARGET = new Set(['ref-279','ref-280','ref-281']);
...
function extractRefFromPath(file){ const parts=file.split('/'); for(const p of parts){ const m=String(p).match(/^ref-(\d{1,6})\b/i); if(m) return `ref-${m[1]}`; const m2=String(p).match(/^(\d{1,6})\b/); if(m2) return `ref-${m2[1]}`; } return null; }
...
(async()=>{
  ...
  for(const f of all){
    ...
    } catch (e) { failures.push({ file:f, ref, err:String(e.message||e) }); }
  }
  ...
  for(const ref of Array.from(TARGET)){
    ...
    try{
      ...
      for(const url of urls){
        ...
        await conn.query('INSERT INTO media (id,bien_id,type,url,motif_upload,position) VALUES (?,?,?,?,?,?)', [id,bienId,'image',url,'remigration_target_279_281_ascii',p]);
        p += 1;
      }
      await conn.commit();
      db.updated.push({ref, bienId, count:urls.length});
    } catch(e){ try{ await conn.rollback(); } catch {} db.errors.push({ref, err:String(e.message||e)}); }
  }

  const health=[];
  for(const ref of Array.from(TARGET).map(r=>r.toUpperCase()).sort()){
    const [r] = await conn.query('SELECT m.url FROM biens b LEFT JOIN media m ON m.bien_id=b.id WHERE b.reference=? ORDER BY m.position ASC LIMIT 1', [ref]);
    const first = r[0]?.url || null;
    const status = first ? await head(first) : null;
    health.push({ref, first, status});
  }
  await conn.end();
  ...
  fs.writeFileSync('/var/www/dwiraimmobilier.com/public/remigration-target-279-281-ascii-report.json', JSON.stringify(report,null,2));
